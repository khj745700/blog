...
public interface BoardRepository extends JpaRepository<Board, Integer> {
    ...
    @Query("SELECT new shop.khj745700.backend.domain.board.rest.dto.BoardView(b) FROM HashTag ht LEFT JOIN HashTagList htl ON ht = htl.hashTag LEFT JOIN Board b ON htl.board = b WHERE ht.id = :id AND b.isHidden = false AND b.wroteDate IS NOT NULL ORDER BY b.wroteDate DESC ")
